...
def train(model, pde_model, config: ModelConfig,
          train_dl: DataLoader, dev_dl: DataLoader,
          compute_strain: torch_ad.Function, compute_sn: torch_ad.Function):
    ...
    for epoch_num in trange(config.epochs):
        ...
        Architecture:
            For each cp_i in dataset:
                ...
                For each time step n:
                    ...
                    # Sketch of the embeddings
                    1)
                    <--------------------------------------------PyTorch (Training)------------------------------------------------------------>
                           <-----------------------------------------------Firedrake--------------------------------------------------->
                                                        <---PyTorch (ML model)--->
                    u_n_P -> u_n -> [Compute strain] -> ϵ_n -> [ML model] -> σ_n (+ u_n) -> [Solve PDE for cp_i] --> u_(n+1) ->... --> ℒ_n -> ℒ_n_P


                    2)
                    <-----------------------------------------------PyTorch (Training)--------------------------------------------------->
                                     <---Firedrake--->      <--Const model-->              <--PDE model-->  
                    u_n_P -> u_n -> [Compute strain] -> ϵ_n -> [ML model] -> σ_n (+ u_n) -> [PDE solver] --> u_(n+1) --> ... --> ℒ_n -> ℒ_n_P

                    Stage 1: Train the PDE model
                        - Dataset: (σ_i, u_i)_{i}
                        - Architecture: Any ML model
                        - Training: ℒ_PDE = ||u - u_exact||_{L2}**2
                    Stage 2: Train the Const model
                        - Dataset: (cp_i, rs_i^n, as_i^n)_{i, n}
                        - Architecture: Any ML model
                        - Training: ℒ = Σ_{n} ℒ_n, with ℒ_n(S_n) = 0.5 * |ϵS - ϵS_exact|**2 + 0.5 * |ϵA_{i} - ϵA_exact|**2

                    3) Initial idea (WRONG!)
                    <--------------------------------------------PyTorch (Training)--------------------------------------------->
                                     <---Firedrake--->                                <---Firedrake--->  
                    u_n_P -> u_n -> [Compute strain] -> ϵ_n -> [ML model] -> σ_n -> [Solve PDE for cp_i] --> ... --> ℒ_n -> ℒ_n_P

                    # Sketch of the computation graph:
                    u_n -> [Compute strain] -> ϵ_n -> [ML model] -> σ_n -> [Solve PDE for cp_i] (also use u_n as initial guess for Newton solver)
                                                                                   |
                                                                                   v
        ...
        where ℒ_n(S_n) = 0.5 * |ϵS - ϵS_exact|**2 + 0.5 * |ϵA_{i} - ϵA_exact|**2
        and with S_n = (ϵR, ϵA)
        """
        model.train()
        ...
        for step_num, batch in tqdm(enumerate(train_dl), total=train_steps):
            ...
            # Exact radial/axial strains
            epsA_exact = batch.target_eps_A
            epsR_exact = batch.target_eps_R
            conf_pressure = batch.cp
            u_n = config.initial_condition

            epsR = torch.zeros(config.num_time_steps)
            epsA = []
            for n in range(config.num_time_steps):
                # Compute strain: ϵ_n = 0.5 * (∇u + (∇u)^T)
                eps_n = compute_strain(u_n)

                # Forward pass
                σ_n = model(eps_n)

                # -- Perfect scenario -- #
                # Solve PDE for cp_i at time step n
                # u_n = solve_pde(σ_n, conf_pressure)

                # -- Surrogate scenario -- #
                u_n = pde_model(u_n, σ_n, conf_pressure)

                # Compute S_n
                epsR_n, epsA_n = compute_sn(u_n)

                # Store values
                epsR[n] = epsR_n
                epsA[n] = epsA_n

            # Compute ℒ which contains the errors for all the time iterations
            loss = 0.5 * (epsR - epsR_exact)**2 + 0.5 * (epsA - epsA_exact)**2
            # Update ℒ
        ...
        print("\n\n\n If I am printed, training works!!!!!! Now you need to fix evaluation!\n\n")
